...
{
  for (long i = 0; i < (long)data.size();
       i++) { // compute X -> X^{254} on i'th ctxt
    invertSingle(data[i]);
  }
}
void buildAffine(std::vector<PolyType>& binMat,
                 PolyType* binVec,
                 ...
                 const helib::EncryptedArrayDerived<helib::PA_GF2>& ea2)
{
  ...
  binMat.resize(8,
                helib::DoubleCRT(ea2.getContext(),
                                 ea2.getContext().getCtxtPrimes() |
                                     ea2.getContext().getSpecialPrimes()));
  for (long j = 0; j < 8; j++)
    binMat[j] = zzxMat[j];
...
void setPackingConstants(NTL::GF2X& XinSlots,
                         NTL::Mat<NTL::GF2X>& unpacking,
                         helib::EncryptedArrayDerived<helib::PA_GF2>& ea2)
{
  ...
  const helib::EncryptedArrayDerived<helib::PA_GF2>& ea =
      context.getEA().getDerived(helib::PA_GF2());

  // Compute the packing constants, with X in all the slots
  {
    std::vector<NTL::GF2X> slots(ea.size(),
                                 NTL::GF2X(1, 1)); // X in all the slots
    NTL::ZZX tmp;
    ea.encode(tmp, slots); // encode as ZZX
    NTL::conv(XinSlots, tmp);
  } // convert to ZZ2X
  ...
  for (long i = 0; i < e; i++)
    for (long j = 0; j < e; j++) {
      std::vector<NTL::GF2X> slots(ea.size(),
                                   rep(K[i][j])); // K[i][j] in all the slots
      NTL::ZZX tmp;
      ea.encode(tmp, slots);
      NTL::conv(unpacking[i][j], tmp);
    }
}
void packCtxt(std::vector<helib::Ctxt>& to,
              const std::vector<helib::Ctxt>& from,
              const NTL::GF2X& XinSlots)
{
  ...
  long e = ea.getDegree() / 8; // the extension degree
  long nPacked =
      helib::divc(from.size(), e); // How many fully-packed ciphertexts
  ...
  for (long i = 0; i < (long)to.size(); i++) {
    to[i] = from[e * i];
    if (e * i == (long)from.size() - 1)
      break; // check if we are done packing

    helib::Ctxt tmp = from[e * i + 1];
    tmp.multByConstant(NTL::conv<NTL::ZZX>(XinSlots));
    to[i] += tmp;
    if (e * i == (long)from.size() - 2)
      break; // check if we are done packing
...
void unpackCtxt(std::vector<helib::Ctxt>& to,
                const std::vector<helib::Ctxt>& from,
                const NTL::Mat<NTL::GF2X>& unpackConsts)
{
  ...
  if (to.size() == 0)
    to.resize(nUnpacked, helib::Ctxt(helib::ZeroCtxtLike, from[0]));
  else {
    if (nUnpacked > (long)to.size())
      nUnpacked = to.size();
    for (long i = 0; i < (long)to.size(); i++)
      to[i].clear();
  }
  ...
  for (long idx = 0; idx < nPacked; idx++) {
    std::vector<helib::Ctxt> conjugates(
        e,
        from[idx]); // Compute the conjugates, Z^{2^{8j}}
    for (long j = 1; j < e; j++)
...
int main(int argc, char* argv[])
{
  ...
  long mValues[][14] = {
      ...
      {2,
       ...
       12,
       -10,
       100}, // m=13*17*(241) m/phim(m)=1.15   C=69  D=4 E=3
  };
  ...
  if (mValues[cid][8] > 1)
    gens.push_back(mValues[cid][8]);
  if (mValues[cid][9] > 1)
    gens.push_back(mValues[cid][9]);

  ords.push_back(mValues[cid][10]);
  if (abs(mValues[cid][11]) > 1)
    ords.push_back(mValues[cid][11]);
  if (abs(mValues[cid][12]) > 1)
    ords.push_back(mValues[cid][12]);
  // ords.push_back(-16);
  ...
  if (boot) {
    if (cid == 11) {
      ...
      if (mValues[cid][5] > 1)
        mvec.append(mValues[cid][5]);
      if (mValues[cid][6] > 1)
        mvec.append(mValues[cid][6]);
    }
  }
  ...
  for (long i = 1; i < 16; i++) { // rotation along 1st dim by size i*ord/16
    long exp = i * ord / 16;
    long val =
        NTL::PowerMod(context.getZMStar().ZmStarGen(0), exp, m); // val = g^exp
  ...
  helib::EncryptedArrayDerived<helib::PA_GF2> ea2(context,
                                                  sm4Poly,
                                                  context.getAlMod());
  ...
  unsigned char b[16] = {0x01,
                         ...
                         0x32,
                         0x10};
  // unsigned char b[16] = { 0x68, 0x1e, 0xdf, 0x34, 0xd2, 0x06, 0x96, 0x5e,
  ...
  NTL::ZZX tmpEncSelector, tmpEncSelector1, tmpEncSelector2, tmpEncSelector3,
      tmpEncSelector4, tmpEncSelector5, tmpEncSelector6, tmpEncSelector7,
      tmpEncSelector8;
  ...
  PolyType encSelector(tmpEncSelector,
                       context,
                       context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector1(tmpEncSelector1,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector2(tmpEncSelector2,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector3(tmpEncSelector3,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector4(tmpEncSelector4,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector5(tmpEncSelector5,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector6(tmpEncSelector6,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector7(tmpEncSelector7,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType encSelector8(tmpEncSelector8,
                        context,
                        context.getCtxtPrimes() | context.getSpecialPrimes());
  ...
  PolyType affVec1(context,
                   context.getCtxtPrimes() | context.getSpecialPrimes()),
      affVec2(context, context.getCtxtPrimes() | context.getSpecialPrimes());
  buildAffine(affMat1, &affVec1, cc, ea2);
  buildAffine(affMat2, &affVec2, cc1, ea2);

  unsigned char ttcc1[] =
      {0xe7, 0xcb, 0x93, 0x26, 0x4c, 0x9d, 0x3a, 0x71, 0x9f};
  unsigned char ttcc2[] =
      {0x2f, 0x5e, 0xbc, 0x79, 0xf2, 0xe5, 0xcb, 0x97, 0x4f};
  unsigned char ttcc3[] =
      {0xc8, 0x95, 0x2f, 0x5f, 0xbe, 0x78, 0xf1, 0xe6, 0xd0};
  std::vector<PolyType> lccMat1, lccMat2, lccMat3;
  PolyType mVec1(context, context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType mVec2(context, context.getCtxtPrimes() | context.getSpecialPrimes());
  PolyType mVec3(context, context.getCtxtPrimes() | context.getSpecialPrimes());
  ...
  NTL::GF2X
      XinSlots; // "Fully packed" poly with X in all the slots, for packing
  ...
  for (int i = 0; i < 32; i++) {
    std::cout << "round:" << i << ":" << ctxts[0].capacity() << ":"
    ...
    for (int n = 0; n < ctxtnum; n++) {
      ...
      if (i == 31) {
        ...
        helib::Ctxt ctxt1(ctxts[n]), ctxt2(ctxts[n]), ctxt3(ctxts[n]),
            ctxt4(ctxts[n]);
  ...
  std::cout << "##from " << std::ctime(&start_time) << " to "
